...
const TamperingDiffCard = ({ evidence }) => {
  ...
  return (
    <div className={`glass-card rounded-2xl p-6 border ${
      ...
      {(() => {
        const aiExp = details.aiModificationExplanation || details.forensicAnalysis?.aiModificationExplanation || evidence?.aiModificationExplanation;
        if (!aiExp && !isTampered) return null;
